...
-- Client type lookup table based on verified categories.
...
-- Personnel lookup table for verified division/field personnel.
CREATE TABLE IF NOT EXISTS personnel (
    ...
    is_active TINYINT(1) NOT NULL DEFAULT 1,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uniq_personnel_name_type (full_name, client_type_id),
    INDEX idx_personnel_client_type (client_type_id),
    INDEX idx_personnel_district (district_id),
    CONSTRAINT fk_personnel_district
        FOREIGN KEY (district_id) REFERENCES districts(id)
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT fk_personnel_client_type
        FOREIGN KEY (client_type_id) REFERENCES client_types(id)
        ON UPDATE CASCADE ON DELETE CASCADE
);

-- Logbook entries table normalized for verified lookups.
CREATE TABLE IF NOT EXISTS logbook_entries (
    ...
    INDEX idx_logbook_client_type_id (client_type_id),
    INDEX idx_logbook_district (district_id),
    INDEX idx_logbook_school (school_id),
    INDEX idx_logbook_personnel (personnel_id),
    CONSTRAINT fk_logbook_personnel
        FOREIGN KEY (personnel_id) REFERENCES personnel(id)
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT fk_logbook_client_type
        FOREIGN KEY (client_type_id) REFERENCES client_types(id)
